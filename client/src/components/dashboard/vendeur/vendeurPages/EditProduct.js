...
import { fetchProduct, editProduct, fetchCategory, setCategory } from '../../../../redux/slices/vendeurSlice';
...
const EditProduct = () => {
  ...
  const { products, category } = useSelector(state => state.vendeur)
  ...
  useEffect(()=>{
    category && category.map((res)=>{
      data.idCategory === res._id && setCategorie(res.category) 
    })
  })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/updateProduct/${id}`, formData);
            ...
            <i className="mt-4">
              {
                file ? (<img src={file} alt="img" className="col-3 border" />)
                  : <img src={__dirname + data.imgPrincipal} alt="principale" style={{ width: '15%', cursor: 'pointer' }} />
              }
            </i>
            ...
            <i className="d-flex flex-row col-12 justify-content-start mt-4 flex-wrap">
              {
                ...
                ) : data.images && data.images.map((res, i) => <img key={i} src={__dirname + res.filePath} alt="imgs" style={{ width: '15%', cursor: 'pointer' }} />)
              }
            </i>
